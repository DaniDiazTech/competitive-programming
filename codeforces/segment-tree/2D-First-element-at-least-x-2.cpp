...
struct stree{
  ...
  int query(int k, int left, int x, int lx, int rx){
    // Last level but still in the tree
    if (x >= t.size()) return -1;
    if (lx >= left && rx - lx == 1 &&  t[x] >= k){
      return lx;
    }
    int ans = -1;
    if (left < m(lx ,rx)){
      if (t[l(x)]>=k)
        ans = query(k, left, l(x), lx, m(lx, rx));
    }
    if (ans != -1) return ans;
    // Go the right
    if (t[r(x)] >= k){
      ans = query(k, left, r(x), m(lx, rx), rx);
    }
    return ans;
  }
